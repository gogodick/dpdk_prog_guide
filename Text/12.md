# 12. 加密设备库
加密设备库提供了一个加密设备框架，用于管理和供应硬件和软件的加密轮询模式驱动，定义了通用API，支持多种不同的加密操作。现在这个框架只加密，认证，链式加密认证和AEAD对称加密操作。
...
```
### 12.2.4. Configuration of Queue Pairs
...
```
### 12.2.5. Logical Cores, Memory and Queues Pair Relationships
...
## 12.3. Device Features and Capabilities
...
### 12.3.2. Device Operation Capabilities
...
### 12.4.1. Enqueue / Dequeue Burst APIs
...
uint16_t rte_cryptodev_dequeue_burst(uint8_t dev_id, uint16_t qp_id,
                                     struct rte_crypto_op **ops, uint16_t nb_ops)
```
### 12.4.2. Operation Representation
...
### 12.4.3. Operation Management and Allocation
...
```
## 12.5. Symmetric Cryptography Support
